...
namespace BizHawkExternalTool.Data
{
    ...
    class InputCollector
    {
        ...
        public InputCollector(IMemoryDomains memoryDomains, IEmulator emulator)
        {
            ...
            short index = 1;
            for (int iy = -INPUT_HEIGHT_WIDTH * 16; iy <= INPUT_HEIGHT_WIDTH * 16; iy = iy + 16)
            {
                for (int ix = -INPUT_HEIGHT_WIDTH * 16; ix <= INPUT_HEIGHT_WIDTH * 16; ix = ix + 16)
                {
        ...
        public void RefreshInputs()
        {
            ...
            _charX = _memoryBase.ReadS16Little(Convert.ToInt32("0x94", 16));
            _charY = _memoryBase.ReadS16Little(Convert.ToInt32("0x96", 16));
            ...
            foreach(var tile in _tileList)
            {
                ...
                if (tile.Value == 1 && (_charY + tile.Key.Y) < 432)
                {
                ...
                {
                    int distX = Math.Abs(sprite.X - (_charX + tile.Key.X));
                    int distY = Math.Abs(sprite.Y - (_charY + tile.Key.Y));
                ...
                {
                    int distX = Math.Abs(extendedSprite.X - (_charX + tile.Key.X));
                    int distY = Math.Abs(extendedSprite.Y - (_charY + tile.Key.Y));
        ...
        private void GetTiles()
        {
            ...
            {
                int currentX = (int)Math.Floor((double)(_charX + tile.X + 8) / 16);
                int currentY = (int)Math.Floor((double)(_charY + tile.Y) / 16);
