...
namespace BizHawkExternalTool.Data
{
    public class MemoryBase
    {
        ...
        public byte[] GetMemoryDump(string domain = null)
        {
            ...
            var memoryField = typeof(LibWaterboxCore.WaterboxMemoryDomain).GetField("_data", BindingFlags.NonPublic | BindingFlags.GetField | BindingFlags.Instance);
            memoryPtr = (System.IntPtr)memoryField.GetValue(MemoryDomains[currentDomain]);

            var monitorField = typeof(LibWaterboxCore.WaterboxMemoryDomain).GetField("_monitor", BindingFlags.NonPublic | BindingFlags.GetField | BindingFlags.Instance);
